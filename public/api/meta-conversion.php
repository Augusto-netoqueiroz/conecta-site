...
function accessToken(): string
{
    ...
    return is_readable($secretFile) ? trim((string) file_get_contents($secretFile)) : '';
}
...
function allowedHost(?string $url): bool
{
    ...
    return in_array($host, ['planostvsky.com.br', 'www.planostvsky.com.br'], true);
}

function cleanCustomData($value): array
{
    if (!is_array($value)) return [];
    $clean = [];
    foreach (array_slice($value, 0, 20, true) as $key => $item) {
        if (!is_string($key) || !preg_match('/^[a-zA-Z0-9_]{1,40}$/', $key) || !is_scalar($item)) continue;
        $clean[$key] = is_string($item) ? substr($item, 0, 200) : $item;
    }
    return $clean;
}
...
function hashedUserData($value): array
{
    if (!is_array($value)) return [];
    $data = [];

    $phone = preg_replace('/\D/', '', (string) ($value['phone'] ?? '')) ?? '';
    if (strlen($phone) === 10 || strlen($phone) === 11) $phone = '55' . $phone;
    if ($phone !== '') $data['ph'] = [hashValue($phone)];
    ...
    foreach ($fields as $source => $destination) {
        $normalized = normalizeText((string) ($value[$source] ?? ''));
        if ($normalized !== '') $data[$destination] = [hashValue($normalized)];
    }

    $zip = preg_replace('/\D/', '', (string) ($value['zip'] ?? '')) ?? '';
    if ($zip !== '') $data['zp'] = [hashValue($zip)];
    if ($data !== []) $data['country'] = [hashValue('br')];
    return $data;
}
...
if ($contentLength > MAX_BODY_BYTES) respond(413, ['ok' => false, 'error' => 'payload_too_large']);

$raw = file_get_contents('php://input', false, null, 0, MAX_BODY_BYTES + 1);
if ($raw === false || strlen($raw) > MAX_BODY_BYTES) respond(413, ['ok' => false, 'error' => 'payload_too_large']);

$payload = json_decode($raw, true);
if (!is_array($payload)) respond(400, ['ok' => false, 'error' => 'invalid_json']);

$eventName = (string) ($payload['event_name'] ?? '');
$eventId = (string) ($payload['event_id'] ?? '');
$eventSourceUrl = (string) ($payload['event_source_url'] ?? '');

if (!in_array($eventName, ['PageView', 'Contact', 'Lead'], true)) {
...
$token = accessToken();
if ($token === '') respond(503, ['ok' => false, 'error' => 'meta_capi_not_configured']);
...
foreach (['fbp', 'fbc'] as $key) {
    ...
    if ($value !== '' && strlen($value) <= 255) $userData[$key] = $value;
}
$externalId = trim((string) ($payload['external_id'] ?? ''));
if ($externalId !== '' && strlen($externalId) <= 255) $userData['external_id'] = [hashValue(strtolower($externalId))];
$userData = array_merge($userData, hashedUserData($payload['user_data'] ?? []));
...
$event = [
    ...
    'user_data' => $userData,
    'custom_data' => cleanCustomData($payload['custom_data'] ?? []),
];
...
$curl = curl_init(sprintf('https://graph.facebook.com/%s/%s/events', META_GRAPH_VERSION, META_PIXEL_ID));
